...
function Get-WcdDeviceManagerDescriptor {
    <#
    .SYNOPSIS
        Declares what Config-DeviceManager contributes to the run.
    ...
    .EXAMPLE
        Get-WcdDeviceManagerDescriptor -ExecutionOptions $options -Config $config -Translations $T
    #>
    ...
    return [pscustomobject]@{
        Name     = 'Config-DeviceManager'
        Order    = 70
        RowOrder = 60
        Steps    = @(
            @{ Key = 'DeviceManagerStatus'; Label = 'Device Manager' }
        )
        Rows     = @(
            @{ Label = $Translations.Checklist.DeviceManager; Steps = @('DeviceManagerStatus') }
        )
        Invoke   = {
            param($ctx)

            Set-WcdDeviceManagerStatus -LogPath $ctx.LogPath -ProgressCallback $ctx.ProgressCallback
        }
    }
}