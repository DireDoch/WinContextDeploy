...
function Get-WcdBitLockerDescriptor {
    <#
    .SYNOPSIS
        Declares what Config-BitLocker contributes to the run.

    .DESCRIPTION
        Two Steps, two rows: a TPM that is not ready and an unencrypted drive are
        answered differently, and the first often explains the second.
    ...
    .EXAMPLE
        Get-WcdBitLockerDescriptor -ExecutionOptions $options -Config $config -Translations $T
    #>
    ...
    return [pscustomobject]@{
        Name     = 'Config-BitLocker'
        Order    = 90
        RowOrder = 80
        Steps    = @(
            @{ Key = 'TpmReadiness';     Label = 'TPM readiness' }
            @{ Key = 'BitLockerStatus';  Label = 'Drive encryption' }
        )
        Rows     = @(
            @{ Label = $Translations.Checklist.Tpm;       Steps = @('TpmReadiness') }
            @{ Label = $Translations.Checklist.BitLocker; Steps = @('BitLockerStatus') }
        )
        Invoke   = {
            param($ctx)

            Set-WcdBitLockerStatus -Elevated $ctx.Elevated -LogPath $ctx.LogPath -ProgressCallback $ctx.ProgressCallback
        }
    }
}